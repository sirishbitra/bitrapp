 ...
 * List of Contacts
 */
exports.listByAuth = function(req, res) {
	Contact.find({'createdBy': req.user}).sort('-created').populate('user', 'displayName').exec(function(err, contacts) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
...
exports.hasAuthorization = function(req, res, next) {
	if (req.contact.createdBy.toString() !== req.user._id.toString()) {
